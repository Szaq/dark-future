...
findMatch regexString inputString =
    find All (regexString |> regex |> caseInsensitive) inputString |> List.head


findAndExtractMatch : String -> String -> Maybe String
findAndExtractMatch regexString inputString =
    case findMatch regexString inputString of
        Just match ->
...
parseLookAt input =
    Maybe.oneOf
        [ findAndExtractMatch "^look at (.*)" input  
                |> Maybe.map (\item -> Look (Item item))
        , findMatch "^look" input `andThen` (\_ -> Just <| Look Place)
        ]
